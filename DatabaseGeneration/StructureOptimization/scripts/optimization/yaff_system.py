...
#
# --
'''Representation of molecular systems with a log object instead of global log'''
...
class System(object):
    ...
    def from_file(cls, *fns, **user_kwargs):
        ...
        """
        cls_log = user_kwargs['log']
        with cls_log.section('SYS'):
            kwargs = {}
            for fn in fns:
                if fn.endswith('.xyz'):
                ...
                if cls_log.do_high:
                    cls_log('Read system parameters from %s.' % fn)
    ...
    def from_hdf5(cls, f, log=None):
        ...
        if cls_log.do_high:
            cls_log('Read system parameters from %s.' % f.filename)
        return cls(**kwargs)
