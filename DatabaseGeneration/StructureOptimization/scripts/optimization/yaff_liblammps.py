...
from yaff_lammpsio import *
from yaff_ff import ForceField, ForcePart, ForcePartTailCorrection, ForcePartGrid, ForcePartPressure, ForcePartValence, ForcePartPair
from yaff.pes.ext import Cell, PairPotEI
from yaff.pes.nlist import BondedNeighborList
...
class ForcePartLammps(ForcePart):
    ...
    def __init__(self, ff, fn_system, fn_log="none", suffix='',
        ...
        ForcePart.__init__(self, 'lammps', self.system, log = self.log, timer = self.timer)
        self.comm = comm
...
def swap_noncovalent_lammps(ff, **kwargs):
    ...
    if not os.path.isfile(fn_table) or overwrite_table:
        ...
        if comm is None or comm.Get_rank()==0:
            ff_tabulate = ForceField(ff.system, parts_tabulated, nlist=ff.nlist, log = ff.log, timer = ff.timer)
            write_lammps_table(ff_tabulate,fn=fn_table, nrows=nrows, log = ff.log)
    ...
    if comm is None or comm.Get_rank()==0:
        write_lammps_system_data(ff.system, ff=ff, fn=fn_system, triclinic=triclinic, log = ff.log)
        ...
        if part.__class__==ForcePartPair:
            ...
            if np.any(part_scalings!=scaling_rules):
                ...
                if part.name=='pair_ei':
                    ...
                    part_correction = ForcePartPair(ff.system, scaling_nlist, correction_scalings, pair_correction, log = ff.log, timer = ff.timer)
                else:
                    part_correction = ForcePartPair(ff.system, scaling_nlist, correction_scalings, part.pair_pot, log = ff.log, timer = ff.timer)
                parts.append(part_correction)
                nlist = scaling_nlist
    # Construct the new force field
    ff_lammps = ForceField(ff.system, parts, nlist=nlist, log = ff.log, timer = ff.timer)
    return ff_lammps
